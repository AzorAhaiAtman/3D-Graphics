...
/* clean up, free memory */
	...
	//free framebuffer first followed by display
	free(display->fbuf);
	display->fbuf = NULL;
...
/* write pixel values into the display */
	...
	//check if x,y values are within screen limits
	if(i<0||j<0||i>=display->xres||j>=display->yres)
		return GZ_SUCCESS;

	rgb_max = 4095;
	
	//If the value of R,G,B is greater than 4095, cap it to 4095
	if(r > rgb_max)
		r = rgb_max;
	if(g > rgb_max)
		g = rgb_max;
...
int GzFlushDisplay2FrameBuffer(char* framebuffer, GzDisplay *display)
{
	...
	*/
	...
	{										
			...
			//left shift bits from 12-bit color to 8-bit color
			red = display->fbuf[i].red >> 4;
			green = display->fbuf[i].green >> 4;
			blue = display->fbuf[i].blue >> 4;
			
			//each color is stored at a multiple of 3 times the index. i.e red = 3i, green = 3i + 1, blue = 3i + 2
			//ordering in R,G,B fortunately worked for me. Don't know how.

			char* red_address = &(framebuffer[3 * i]);			
			char* green_address = red_address + 1;
			char* blue_address = red_address + 2; 
