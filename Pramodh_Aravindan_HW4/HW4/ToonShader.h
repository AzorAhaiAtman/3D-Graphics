...
GzIntensity clampGzIntensity(GzIntensity val);
...
void toonShading(GzDisplay *display)
{
	...
	for (int p=0; p<XRES; p++)
		for (int q=0; q<YRES; q++)
		{
			float component = clampGzIntensity(sqrt(float((rsI[p+q*XRES]*rsI[p+q*XRES]) +  (rsI2[p+q*XRES]*rsI2[p+q*XRES]))));
			if(component > 3000)
			{
				display->fbuf[(p)+(q)*display->xres].red= 4095 - component;
				display->fbuf[(p)+(q)*display->xres].green= 4095 - component;
				display->fbuf[(p)+(q)*display->xres].blue= 4095 - component;
			}
			else
			{
				display->fbuf[(p)+(q)*display->xres].red= quantize4(display->fbuf[(p)+(q)*display->xres].red);		//or use quantize4
				display->fbuf[(p)+(q)*display->xres].green= quantize5(display->fbuf[(p)+(q)*display->xres].green);
				display->fbuf[(p)+(q)*display->xres].blue= quantize4(display->fbuf[(p)+(q)*display->xres].blue);
			}
		}
}
